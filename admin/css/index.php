...
?><!DOCTYPE html><html lang="pt-br">    <head>        <meta charset="utf-8"/>        <title>Em Duvida - administração</title>        <link rel="stylesheet" type="text/css" href="css/style.css"/>        <script src="js/jscolor/jscolor.js"></script>        <script src="js/jquery-1.11.3.min.js"></script>        <script src="js/js.js"></script>    </head>    <body><?php if (isset($_SESSION['email_usuario']) and $_SESSION['tipo_usuario'] == 2) { ?>            <?php
            ...
            ?>        <?php } else { ?>        <form method="post" class="formulario" action="" id="logAdmin">                <label for="email">E-mail:</label>                <input type="email" name="email" class="input" id="email"/>                <label for="senha">Senha:</label>                <input type="password" name="senha" class="input" id="senha"/>                <input type="submit" value="Entrar" name="logar"/>            </form><?php } ?>        <?php
